    ...
    {loading ? <ContentLoader label="Preparing analysis workspace" minHeight={620} /> : error && !activeCall ? <Alert type="error" showIcon title="Unable to open analysis" description={error} /> : activeCall && <div className="analysis-workspace data-reveal">
      ...
      <div className="analysis-workspace__columns">
        ...
        <section className={`analysis-panel analysis-panel--form${editable || completed ? '' : ' is-locked'}`} aria-label="QA evaluation form">
          <div className="analysis-panel__heading"><span><FormOutlined /></span><div><strong>QA evaluation</strong>{Boolean(completed || revisionRequired || lockedByAnother) && <small>{completed ? `${review?.rating_label} · ${review?.outcome_label}` : revisionRequired ? 'Reassessment in progress' : `Locked by ${reservation?.reviewer_name}`}</small>}</div></div>
          {!editable && !completed ? <div className="analysis-form-placeholder"><span className="analysis-form-placeholder__icon"><LockOutlined /></span><strong>{lockedByAnother ? 'This call is reserved' : 'Reserve to unlock the scorecard'}</strong>{lockedByAnother && <Text type="secondary">{reservation?.reviewer_name} currently owns this analysis.</Text>}</div> : scorecard && <Form form={form} layout="vertical" className="qa-scorecard" disabled={!editable}>
            <div className="qa-call-classification">
              <div><strong>Evaluation scope</strong></div>
