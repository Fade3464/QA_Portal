...
import type { AdminSummary, AdminUserRecord, BranchRecord, CompanyRecord, DialerRecord, SecurityEvent } from '../types';

const { Title, Paragraph, Text } = Typography;
type Resource = 'companies' | 'branches' | 'users' | 'dialers';
type ManagedRecord = CompanyRecord | BranchRecord | AdminUserRecord | DialerRecord;
...
function AdministrationOverview({ summary, events, loading, onNavigate }: { summary: AdminSummary | null; events: SecurityEvent[]; loading: boolean; onNavigate: (key: string) => void }) {
  const cards = [
    ...
    { label: 'Calls ingested', value: summary?.calls ?? 0, detail: 'All time', icon: <CheckCircleFilled />, tab: '' },
  ];
  return <div className="admin-overview">
    <Row gutter={[16, 16]}>{cards.map((card) => <Col xs={24} sm={12} xl={6} key={card.label}><Card hoverable className="admin-stat" loading={loading} onClick={() => card.tab && onNavigate(card.tab)}><span className="admin-stat__icon">{card.icon}</span><Statistic title={card.label} value={card.value} /><Text type="secondary">{card.detail}</Text></Card></Col>)}</Row>
    <Row gutter={[16, 16]}>
...
function EditorDrawer({ editor, form, saving, companies, branches, selectedCompany, onClose, onSave }: { editor: EditorState; form: FormInstance; saving: boolean; companies: CompanyRecord[]; branches: BranchRecord[]; selectedCompany?: string; onClose: () => void; onSave: (values: Record<string, unknown>) => Promise<void> }) {
