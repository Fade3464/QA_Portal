...
function EditorDrawer({ editor, form, saving, companies, branches, users, dialers, selectedCompany, selectedBranch, selectedRole, onClose, onSave }: { editor: EditorState; form: FormInstance; saving: boolean; companies: CompanyRecord[]; branches: BranchRecord[]; users: AdminUserRecord[]; dialers: DialerRecord[]; selectedCompany?: string; selectedBranch?: string; selectedRole?: AdminUserRecord['role']; onClose: () => void; onSave: (values: Record<string, unknown>) => Promise<void> }) {
  ...
  return <Drawer open={Boolean(editor)} onClose={onClose} size="large" destroyOnHidden title={<div className="drawer-title"><span className="drawer-title__icon">{resource === 'teams' ? <ClusterOutlined /> : resource === 'users' ? <TeamOutlined /> : resource === 'dialers' ? <ApiOutlined /> : resource === 'branches' ? <ApartmentOutlined /> : <BankOutlined />}</span><span><strong>{isEdit ? 'Edit' : 'Add'} {resource ? resourceLabels[resource].singular : ''}</strong><small>{isEdit ? 'Update configuration and access status.' : 'Complete the details to create this record.'}</small></span></div>} extra={<Space><Button onClick={onClose}>Cancel</Button><Button type="primary" loading={saving} onClick={() => form.submit()}>Save changes</Button></Space>}>
    <Form form={form} layout="vertical" requiredMark="optional" onFinish={onSave} className="admin-form">
      ...
      {resource === 'branches' && <><Form.Item name="company" label="Company" rules={[{ required: true }]}><Select showSearch={{ optionFilterProp: 'label' }} options={companyOptions} placeholder="Select company" /></Form.Item><Form.Item name="name" label="Branch name" rules={[{ required: true }]}><Input placeholder="e.g. New York" /></Form.Item><Form.Item name="code" label="Branch code" rules={[{ required: true }, { pattern: /^[a-z0-9-]+$/ }]}><Input placeholder="nyc" /></Form.Item><Form.Item name="timezone" label="Timezone" rules={[{ required: true }]} tooltip="Eastern Time automatically follows EST and EDT daylight-saving transitions."><Select options={[{ value: 'America/New_York', label: 'Eastern Time — New York (EST/EDT)' }]} /></Form.Item></>}
...
function ProjectAccessDrawer({ user, form, dialers, saving, onClose, onSave }: { user: AdminUserRecord | null; form: FormInstance; dialers: DialerRecord[]; saving: boolean; onClose: () => void; onSave: (values: { project_assignment_ids?: string[] }) => Promise<void> }) {
  ...
  >
    {user && <div className="project-access-drawer">
      <Alert type="info" showIcon title={`Projects available in ${user.branch_name}`} description={user.role === 'project_manager' ? 'The Project Manager will only see calls, QA reports, and performance analytics from the selected projects.' : user.role === 'qa' ? 'The QA analyst will only see calls and reports from the selected projects.' : 'The Team Leader will only see calls and QA reports from the selected projects and the teams they lead.'} />
      ...
      <Form form={form} layout="vertical" requiredMark="optional" onFinish={onSave} clearOnDestroy>
        <Form.Item name="project_assignment_ids" label="Assigned projects" extra={`Select one or more projects. Removing a project immediately removes it from this ${roleLabel}'s access scope.`} rules={user.role === 'team_leader' ? [] : [{ required: true, message: 'Assign at least one project.' }]}>
