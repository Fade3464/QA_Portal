...
function outcomeColor(report: QAReport) {
  ...
  if (report.score === null) return 'default';
...
function scoreDisplay(report: QAReport) {
  if (report.critical_errors.length) return 'FAIL';
  return report.score === null ? 'N/A' : `${report.score}%`;
}
...
function ReportManagementDrawer({ report, loading, canManage, oversight, actionStatus, actionNote, coachingDue, saving, rangeRequest, onClose, onStatusChange, onNoteChange, onDueChange, onSubmit, onPlayPatch }: ReportManagementDrawerProps) {
  ...
  const scorecardItems = report && snapshot?.categories ? snapshot.categories.map((category) => {
    ...
    const entries = category.criteria.filter((criterion) => Object.hasOwn(report.scores, criterion.key) || Boolean(report.criterion_evidence?.[criterion.key]) || report.criterion_applicability?.[criterion.key] === 'not_reached');
    if (!entries.length && applicability === 'applicable') return null;
    const subtotal = category.criteria.reduce((sum, criterion) => sum + Number(report.scores[criterion.key] ?? 0), 0);
    const applicableMaximum = applicability === 'applicable'
      ? category.criteria.reduce((sum, criterion) => sum + (report.criterion_applicability?.[criterion.key] === 'not_reached' ? 0 : criterion.max_score), 0)
      : category.max_score;
    const patchCount = entries.reduce((sum, criterion) => sum + (report.criterion_evidence?.[criterion.key]?.patches.length ?? 0), 0);
    const categoryStatus = applicability === 'not_reached' ? 'Not reached' : applicability === 'missed_opportunity' ? 'Missed · 0' : applicableMaximum > 0 ? `${subtotal}/${applicableMaximum}` : 'No applicable items';
    return {
      key: category.key,
    ...
    {loading ? <ContentLoader label="Opening QA report" minHeight={480} /> : report && <div className="tl-report-detail data-reveal">
      ...
      {canManage && <Card size="small" className="tl-action-card" title={<span><strong>Management decision</strong></span>}>
        <Form layout="vertical">
          ...
          {actionStatus === 'returned_to_qa' && <Alert className="tl-return-alert" type="warning" showIcon title="Return this report for reassessment" description={`The report will leave your active queue and return to ${report.reviewer_name} with your feedback.`} />}
...
function ReportContents({ report, snapshot }: { report: QAReport; snapshot?: QAScorecard }) {
  return <div className="report-detail"><div className={`report-detail__score${report.critical_errors.length ? ' is-critical' : ''}`}><Progress type="dashboard" percent={Number(report.score ?? 0)} status={report.critical_errors.length ? 'exception' : report.score !== null && Number(report.score) >= 85 ? 'success' : 'normal'} format={() => scoreDisplay(report)} /><span><Tag color={outcomeColor(report)}>{report.rating_label || report.status_label}</Tag><strong>{report.outcome_label}</strong><small>{report.agent_name} · {report.team_name} · {report.coverage ?? 0}% coverage</small></span></div>{snapshot?.categories?.map((category) => { const applicability = report.category_applicability?.[category.key] ?? 'applicable'; const subtotal = category.criteria.reduce((sum, criterion) => sum + Number(report.scores[criterion.key] ?? 0), 0); const applicableMaximum = applicability === 'applicable' ? category.criteria.reduce((sum, criterion) => sum + (report.criterion_applicability?.[criterion.key] === 'not_reached' ? 0 : criterion.max_score), 0) : category.max_score; return <section className="report-category-block" key={category.key}><div className="report-category"><span><strong>{category.label}</strong><small>{applicability === 'not_reached' ? 'Not reached' : applicability === 'missed_opportunity' ? 'Missed opportunity · 0' : applicableMaximum > 0 ? `${subtotal}/${applicableMaximum}` : 'No applicable items'}</small></span>{applicability !== 'not_reached' && applicableMaximum > 0 && <Progress percent={(subtotal / applicableMaximum) * 100} showInfo={false} size="small" />}</div></section>; })}{report.feedback_summary && <ReportText label="Summary" value={report.feedback_summary} />}{report.strengths && <ReportText label="Strengths" value={report.strengths} />}{report.expected_behavior && <ReportText label="Expected behavior" value={report.expected_behavior} />}{report.coaching_plan && <ReportText label="Coaching / follow-up" value={report.coaching_plan} />}<div className="report-detail__meta"><Text type="secondary">Reviewed by {report.reviewer_name}</Text><Text type="secondary">{report.completed_at ? `${appDate(report.completed_at).format('DD MMM YYYY, h:mm A')} ET` : 'Draft'}</Text></div></div>;
}
